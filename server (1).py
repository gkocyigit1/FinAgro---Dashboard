...
class Handler(BaseHTTPRequestHandler):
    ...
    def _parse_upload(self):
        """Multipart form-data'dan dosyayı çıkar"""
        ct  = self.headers.get('Content-Type', '')
        ln  = int(self.headers.get('Content-Length', 0))
        body = self.rfile.read(ln)
        # boundary'yi bul - tırnak işaretlerini temizle
        bnd = ct.split('boundary=')[-1].strip().strip('"').encode()
        ...
        for part in parts:
            if b'filename=' in part and b'\r\n\r\n' in part:
                ...
                if content:
                    return content
        raise ValueError('Dosya bulunamadi - boundary: ' + bnd.decode())

    def do_POST(self):
        p = urlparse(self.path).path
        if p=='/api/upload':
            try:
                fd = self._parse_upload()
